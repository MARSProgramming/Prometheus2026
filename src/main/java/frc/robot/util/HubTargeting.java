...
public class HubTargeting {
     ...
     */
    public HubTargeting(double marginPid, double marginSetpoint, double kP, double kD, double velocityCompensationFactor) {
        pidMargin = marginPid;
    ...
    public ChassisSpeeds getHubTargetSpeeds(Pose2d currentPose) {
        ...
        // Difference in target positions
        Pose2d targetPose = FieldConstants.HubPoses.getHubPose().toPose2d();
        deltaTranslation = targetPose.getTranslation().minus(correctedPose.getTranslation());

        FieldConstants.HubPoses.getHubPose();
//            targetPose = new Pose2d(new Translation2d(targetPose.getX(), FIELD_WIDTH - 1), new Rotation2d()) ;
        ...
        SmartDashboard.putNumber("AutoAim/RotationalError", rotateToTarget.getDegrees());
        ...
        && Math.abs(robotSpeed.omegaRadiansPerSecond) < FieldConstants.AutoAim.AUTO_AIM_MAX_VEL_SETPOINT;
    ...
    public boolean isInSetpointMargin() { 
        return isInSetpointMargin; 
    }
    ...
    private Pose2d compensateRobotPose(Pose2d robotPose, ChassisSpeeds robotSpeed){
        ...
        // rough velocity compensation

        deltaPos = new Translation2d(
            robotSpeed.vxMetersPerSecond * velocityCompensationFactor, 
            robotSpeed.vyMetersPerSecond * velocityCompensationFactor);
